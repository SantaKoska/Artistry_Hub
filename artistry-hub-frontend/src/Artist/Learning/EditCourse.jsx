...
const EditCourse = ({ selectedCourse }) => {
  ...
  const handleEditCourse = async () => {
    try {
      const res = await axios.put(
        `http://localhost:8000/artist/edit-course/${courseData._id}`,
        editedCourse,
        {
          headers: {
  ...
  const handleAddChapter = async () => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:8000/artist/add-chapter/${courseData._id}`,
        newChapter,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDeleteChapter = async (chapterId) => {
    try {
      const res = await axios.delete(
        `http://localhost:8000/artist/delete-chapter/${courseData._id}/${chapterId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleEditChapter = async (
    ...
    try {
      const res = await axios.put(
        `http://localhost:8000/artist/edit-chapter/${courseData._id}/${chapterId}`,
        {
          title: updatedTitle,
  ...
  const handleAddLesson = async (chapterId) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:8000/artist/add-lesson/${courseData._id}/${chapterId}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDeleteLesson = async (chapterId, lessonId) => {
    try {
      const res = await axios.delete(
        `http://localhost:8000/artist/delete-lesson/${courseData._id}/${chapterId}/${lessonId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleEditLesson = async (chapterId, lessonId, updatedLesson) => {
    ...
    try {
      const res = await axios.put(
        `http://localhost:8000/artist/edit-lesson/${courseData._id}/${chapterId}/${lessonId}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
                      ...
                      <div className="mb-2">
                        ...
                        {lesson.mediaUrl && (
                          <div className="mb-2">
                            <video
                              controls
                              src={`http://localhost:8000${lesson.mediaUrl}`}
                        ...
                        {lesson.noteUrl && (
                          <div className="mb-2">
                            <iframe
                              src={`http://localhost:8000${lesson.noteUrl}`}
